...
get_imdb_file <- function(fname){
    ...
    if(file.exists(rds_file)){
        # Load the data from the saved .rds file
        return(readRDS(rds_file))
    } else {
        # If the file doesn't exist, download and save the .rds file
        if(!file.exists(fname_ext)){
            FILE_URL <- paste0(BASE_URL, fname_ext)
            download.file(FILE_URL, destfile = fname_ext)
        }
        df <- as.data.frame(readr::read_tsv(fname_ext, lazy = FALSE))
        saveRDS(df, rds_file)  # Save the data as an .rds file for future use
        return(df)
    }
}
...
happy_days_title  <- TITLE_BASICS |>
    filter(primaryTitle == "Happy Days")

#Join with episodes and ratings
TITLE_EPISODES |>
    filter(parentTconst == happy_days$tconst) |>
    inner_join(TITLE_RATINGS, by = "tconst") |>
...
spielberg_success <- spielberg_movies |> 
    arrange(desc(success_metric)) |> 
    select(primaryTitle, averageRating, numVotes, success_metric)
...
movies_with_genres |> 
    group_by(genres) |> 
    summarise(avg_success = mean(success_metric, na.rm = TRUE), 
              num_movies = n()) |>  # Count how many movies are in each genre
    arrange(desc(avg_success))
...
top_genre_by_decade <- successful_movies_by_decade |> 
    group_by(decade) |> 
...
print(total_successes_by_genre)
...
genre_success_rate <- movies_genre |> 
    group_by(genre) |> 
    summarise(
        total_movies = n(),
...
genre_popularity_trend <- movies_genre |> 
...
successful_actors <- TITLE_PRINCIPALS |> 
    ...
    group_by(nconst) |> 
    summarise(num_successful_movies = sum(success_metric >= success_threshold), 
...
ggplot(key_talent, aes(x = reorder(primaryName, -num_successful_movies), y = num_successful_movies)) +
    ...
    labs(title = "Number of Successful Movies for Key Talent",
         x = "Talent",
...
actors <- successful_actors |> head(2) |> mutate(role = "Actor")
directors <- successful_directors |> head(1) |> mutate(role = "Director")
...
classic_movies <- movies_ratings |> 
    filter(startYear < year_cutoff, 
           averageRating >= 7.5, 
           numVotes >= 50000) |> 
    arrange(desc(averageRating))

# View the top candidates for a remake
print(classic_movies |> head(10))
```
