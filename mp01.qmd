...
### Description
Following page will analyze Fiscal Characteristics of Major US Public Transit Systems using publicly available data. Description of this problem can be found here:   [Mini-Project #01](https://michael-weylandt.com/STA9750/miniprojects/mini00.html). 
...
## Load Data
Since we have now setup libraries, we will now download the data to our project so that we can use later. You might get an error when trying to download the file programmatically. If the error persists, download the files manually and rename those and copy them to project folder. 
...
#### 2.  Display the result nicely using DT and format the number
...
#### 3. Agencies with Largest Decrease in Ridership
```{r}
...
```{r}
  ...
  formatRound("Farebox_Recovery", digits = 0, mark = ",")
```
...
```{r}
  ...
  formatRound("Expenses_per_UPT", digits = 0, mark = ",")
...
```{r}
  USAGE_AND_FINANCIALS |>
   ...
   formatRound("Fares_per_UPT", digits = 0, mark = ",")
```
...
```{r}
  USAGE_AND_FINANCIALS |>
   ...
   formatRound("Expenses_per_VRM", digits = 0, mark = ",")
```
...
```{r}
  ...
  formatRound("Fares_per_VRM", digits = 0, mark = ",")
```
:::
...
## Conclusion
This was an interesting assignment to understand basics DT operations using Transportation data. 
