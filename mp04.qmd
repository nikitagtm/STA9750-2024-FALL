...
### Does the Employee Run Out of Funds Before Death (ORP Only)?
...
One of the key questions is whether the ORP balance runs out before the employee passes away.We can check for each simulation if the ORP balance reaches zero before the employee’s death age (age 85).
This will allow us to calculate the probability that the ORP account is exhausted before death.
Code for ORP Exhaustion Probability:
...
###  Evaluate the 4% Withdrawal Rate
